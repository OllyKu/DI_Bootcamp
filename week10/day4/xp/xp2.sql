...
--Write a query which should check if your favorite movie exists in the database. Have your query get the film ID, title, 
--description, length and the rental rate, these details can be found in the “film” table.
--SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Lola';
...
-- SELECT title, replacement_cost FROM film ORDER BY replacement_cost
-- LIMIT 10 OFFSET 10;
...
-- SELECT amount, payment_date FROM customer INNER JOIN payment
-- ON customer.customer_id = payment.customer_id
-- ORDER BY payment.customer_id;

-- You need to check your inventory. Write a query to get all the movies which are not in inventory.
-- SELECT distinct title FROM film LEFT JOIN inventory
